...
export function FacilityUpgrade({ isOpen, facility, onClose }: FacilityUpgradeProps) {
  ...
  const getUpgradeStatus = (upgrade: UpgradeDefinition): 'available' | 'installed' | 'locked' | 'expensive' => {
  ...
  const handleInstallUpgrade = (upgrade: UpgradeDefinition) => {
    ...
    addStatusMessage(
      `Upgrade installed: ${upgrade.name} (${benefits})`,
      'success'
    );
  ...
  const calculateTotalBenefits = () => {
    const totalBenefits = facilityUpgrades.reduce((total, upgradeId) => {
              ...
              <PulseEffect key={upgrade.id} pulse={isSelected} color="green">
                <div
                  className={`
                    ...
                    ${status === 'installed' ? 'border-green-400 bg-green-400/20' :
                      status === 'available' ? 'border-slate-600 bg-slate-800 hover:border-green-400/50' :
                      status === 'expensive' ? 'border-yellow-400/50 bg-yellow-400/10' :
                            ...
                            return (
                              <span
                                ...
                                className={`text-xs px-1 py-0.5 rounded mr-1 ${
                                  isReqMet ? 'bg-green-400/20 text-green-400' : 'bg-red-400/20 text-red-400'
                                }`}
                              >
                                {reqUpgrade?.name || req}
                              </span>
