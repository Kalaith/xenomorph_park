    ...
    <div className="space-y-3">
      ...
      <div className="bg-slate-900/80 border border-green-400/30 rounded-lg p-3">
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {[
            ...
            { mode: 'research' as SortMode, label: 'Research Cost', icon: '🔬' },
            { mode: 'availability' as SortMode, label: 'Available First', icon: '🔓' }
          ].map(({ mode, label, icon }) => (
            <Button
              key={mode}
      ...
      {groups.map((group) => (
        <CollapsiblePanel
          ...
          defaultExpanded={group.category === 'basic' || group.species.some(s => isResearched(s) || isAvailable(s))}
          actions={[
            {
              label: 'Quick Select',
              icon: '⚡',
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        {available && !researched && <span className="text-yellow-400 text-xs">🔓</span>}
                      </div>
