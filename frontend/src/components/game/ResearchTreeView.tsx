...
function ResearchNodeComponent({ node, onNodeClick }: ResearchNodeComponentProps) {
  ...
  const canAfford =
    resources.credits >= node.cost.credits && resources.research >= node.cost.research;
    ...
    >
      ...
      <div className="text-xs space-y-1">
        <div className="flex justify-between">
          <span>💰</span>
          <span
            className={resources.credits >= node.cost.credits ? 'text-green-400' : 'text-red-400'}
          >
        ...
        <div className="flex justify-between">
          <span>🔬</span>
          <span
            className={resources.research >= node.cost.research ? 'text-green-400' : 'text-red-400'}
          >
            {node.cost.research}
          </span>
        </div>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Research: ${node.name}`}>
      <div className="space-y-4">
        ...
        <div className="bg-slate-800 rounded p-3">
          ...
          {nodeState.completed && <span className="text-green-400">✅ Completed</span>}
          {nodeState.inProgress && (
            <div>
          ...
          <div className="bg-slate-800 rounded p-3">
            ...
            <ul className="space-y-1">
              {node.prerequisites.map(prereqId => {
                const prereqNode = researchTreeData.find(n => n.id === prereqId);
          ...
          >
            ...
            >
              ...
              {researchTree.map(node => (
                <ResearchNodeComponent key={node.id} node={node} onNodeClick={handleNodeClick} />
              ))}
            </div>
          </div>
        </div>
