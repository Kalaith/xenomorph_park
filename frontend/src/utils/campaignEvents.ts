...
class CampaignEventManager {
  ...
  private checkTriggerCondition(trigger: EventTrigger, gameState: GameState): boolean {
    switch (trigger.type) {
      ...
      case 'resource':
        return Object.entries(trigger.condition as Record<string, number>).every(
          ([resource, value]) => {
            const current =
              (gameState.resources as unknown as Record<string, number>)[resource] ?? 0;
      ...
      case 'facility':
        ...
        if ((trigger.condition as { hasType?: string }).hasType) {
          return gameState.facilities.some(
            f => f.name === (trigger.condition as { hasType: string }).hasType
          );
        }
        return false;
  ...
  public applyEventChoice(event: CampaignEvent, choice: EventChoice): void {
      ...
      switch (consequence.type) {
        case 'resource': {
          ...
          Object.entries(consequence.effect as Record<string, number>).forEach(
            ([resource, value]) => {
              const currentValue =
                (gameStore.resources as unknown as Record<string, number>)[resource] ?? 0;
        ...
        case 'crisis':
          ...
          gameStore.addStatusMessage(`⚠️ Crisis Event: ${consequence.description}`, 'error');
          break;
      }
    });
    ...
    gameStore.addStatusMessage(`📖 ${event.name}: ${choice.description}`, 'info');
  }
